...
$STORAGE_DIR = $SITE_ROOT . '/admin/_storage'; // NUEVO: directorio protegido
$DB_PATH = $STORAGE_DIR . '/colaboradores_private.json'; // NUEVO: archivo privado
  ...
  if ($honeypot !== '') {
    ...
    if ($error === '') {
      ...
      if (!write_json_atomic($DB_PATH, $db)) {
        $error = 'No se pudo guardar la postulación. Verificá permisos de /admin/_storage.';
      } else {
        $ok = true;
      }
    }
