...
function clean_note_array(array $n): array {
    $out = [];
    $out['id'] = fix_mojibake(trim((string)($n['id'] ?? '')));
    $out['fecha'] = fix_mojibake(trim((string)($n['fecha'] ?? '')));
    $out['provincia'] = fix_mojibake(trim((string)($n['provincia'] ?? '')));
    $out['categoria'] = fix_mojibake(trim((string)($n['categoria'] ?? '')));
    $out['titulo'] = fix_mojibake(trim((string)($n['titulo'] ?? '')));
    $out['texto'] = fix_mojibake(trim((string)($n['texto'] ?? '')));
    $out['imagen'] = fix_mojibake(trim((string)($n['imagen'] ?? ''))); // nuevo
    ...
    if (is_array($links)) {
        foreach ($links as $l) {
            ...
            $href = fix_mojibake(trim((string)($l['href'] ?? '')));
            $label = fix_mojibake(trim((string)($l['label'] ?? '')));
            if ($href !== '' || $label !== '') {
