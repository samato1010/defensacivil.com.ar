...
// === NUEVO: STORAGE PRIVADO ===
if (!defined('STORAGE_DIR')) {
  define('STORAGE_DIR', __DIR__ . '/_storage'); // /public_html/admin/_storage (protegido por .htaccess)
}

// === NUEVO: Archivos separados ===
// Público (solo colaboradores visibles, sin credenciales)
if (!defined('COLAB_PUBLIC_JSON')) {
  define('COLAB_PUBLIC_JSON', DATA_DIR . '/colaboradores_public.json');
}
// Privado (credenciales, items, requests)
if (!defined('COLAB_PRIVATE_JSON')) {
  define('COLAB_PRIVATE_JSON', STORAGE_DIR . '/colaboradores_private.json');
}

// Mantener COLAB_JSON apuntando al privado para retrocompatibilidad con código existente
if (!defined('COLAB_JSON')) {
  define('COLAB_JSON', COLAB_PRIVATE_JSON);
}
...
function ensure_storage(): void {
  ...
  // NUEVO: Separar colaboradores en público y privado
  if (!file_exists(COLAB_PUBLIC_JSON)) {
    ...
    json_write_atomic(COLAB_PUBLIC_JSON, $seed);
  }
  ...
  if (!file_exists(COLAB_PRIVATE_JSON)) {
    ...
    json_write_atomic(COLAB_PRIVATE_JSON, $seed);
  }
...
/* =========================
   Users store (internos + colaboradores privado)
   ========================= */
...
function dc_load_users_from_colaboradores(): array {
  // CAMBIO: ahora lee del archivo PRIVADO
  $data = json_read(COLAB_PRIVATE_JSON, [
    'meta' => [],
    'items' => [],
  ]);
...
/* =========================
   NUEVO: Helper para sincronizar público/privado
   ========================= */

/**
 * Sincroniza colaboradores aprobados desde privado -> público
 * Solo copia: id, nombre, email, provincia, localidad, nivel, rol
 * NO copia: credenciales, IPs, user-agents, etc.
 */
function sync_colaboradores_public(): bool {
  $private = json_read(COLAB_PRIVATE_JSON, ['meta'=>[], 'items'=>[]]);
  
  $collaborators = [];
  foreach ($private['items'] as $item) {
    if (!is_array($item)) continue;
    
    $status = strtolower(trim((string)($item['status'] ?? 'pendiente')));
    
    // Solo sincronizar aprobados
    if ($status === 'aprobado') {
      $collaborators[] = [
        'id' => (string)($item['id'] ?? ''),
  ...
  return json_write_atomic(COLAB_PUBLIC_JSON, $public);
}
