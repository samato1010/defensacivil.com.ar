...
// Verificar si existe security.php, si no usar funciones básicas
...
// Determinar rutas
$PRIVATE_JSON = defined('COLAB_PRIVATE_JSON') ? COLAB_PRIVATE_JSON : __DIR__ . '/_storage/colaboradores_private.json';
$PUBLIC_JSON = defined('COLAB_PUBLIC_JSON') ? COLAB_PUBLIC_JSON : __DIR__ . '/../data/colaboradores_public.json';

// Leer datos
$data = [];
if (file_exists($PRIVATE_JSON)) {
    $content = file_get_contents($PRIVATE_JSON);
    $data = json_decode($content, true) ?: ['meta' => [], 'items' => []];
} else {
    $data = ['meta' => [], 'items' => []];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verificar CSRF si la función existe
    $csrf_ok = true;
    if (function_exists('csrf_check')) {
        $csrf_ok = csrf_check($csrf);
    }
    ...
    if (!$csrf_ok) {
        ...
        if ($action === 'approve') {
            $id = $_POST['id'] ?? '';
            $role = trim($_POST['role'] ?? 'colaborador');
            
            // Permisos
            $perms = [
                'directorio' => isset($_POST['perm_directorio']) ? 1 : 0,
                'historia' => isset($_POST['perm_historia']) ? 1 : 0,
                'notas' => isset($_POST['perm_notas']) ? 1 : 0,
                'colaboradores' => 0
            ];
            ...
            foreach ($items as $idx => $item) {
                if (($item['id'] ?? '') === $id) {
                    ...
                    $items[$idx]['approved_by'] = $_SESSION['user'] ?? 'admin';
            ...
            file_put_contents($PRIVATE_JSON, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
            
            // Sincronizar público si existe la función
        ...
        elseif ($action === 'reject') {
            $id = $_POST['id'] ?? '';
            $motivo = trim($_POST['motivo'] ?? 'Sin especificar');
            ...
            foreach ($items as $idx => $item) {
                if (($item['id'] ?? '') === $id) {
                    ...
                    $items[$idx]['rejected_by'] = $_SESSION['user'] ?? 'admin';
                    $items[$idx]['updated_at'] = date('Y-m-d H:i:s');
...
// Generar CSRF token (si la función existe)
$csrf_token = '';
